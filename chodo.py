...
def add_user(message):
    ...
    if user_id in admin_id:
        ...
        response = "Mood ni hai abhi pelhe purchase kar isse:- @RAJPUTDDOS."
...
 Usage: /remove <userid>'''
        ...
        response = "Purchase karle bsdk:- @RAJPUTDDOS ."
...
def start_bgmi_reply(message, target, port, time):
    ...
    response = f"CHUDAI START : {target}:{port} for {time}\nSEC ATTACK LAG GYA H JAB TAK EYE ATTACK LAGA H DUSRA NAHI LEGA OK 👿👻"
    bot.reply_to(message, response)
...
def handle_bgmi(message):
    ...
    if user_id in allowed_user_ids:
        if bgmi_running:
            response = " EK ATTACK LAGA H PHELE US ATTACK KO FINISH HONE DO OKK 👻👿."
            bot.reply_to(message, response)
            return
        ...
        if len(command) == 4:  # Updated to accept target, port, and time
            ...
            if time > 200:
                response = "Error: Buy From @RAJPUTDDOS For More Time"
            else:
                ...
                try:
                    ...
                    response = f"CHUDAI STOP : {target}:{port} for {time}\nSEC AB NEW ATTACK LAGO OKK 👿👻"
                except Exception as e:
        ...
        response = "BUY KAR LO FREE NAHI H DM TO BUY @RAJPUTDDOS"
...
Buy From :- @RAJPUTDDOS
Official Channel :- https://t.me/+NYbRtL8swZZmNzM1
'''
...
3. MAKE SURE YOU JOINED https://t.me/+NYbRtL8swZZmNzM1 OTHERWISE NOT WORK
